...
export default function EngineerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.append('phoneNumber', formData.phoneNumber); payload.append('role', 'Engineer');
    ...
    <div className="min-h-screen bg-brand flex items-center justify-center py-12 px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="text-center mb-8">
          ...
          <div className="mt-1"><span className="px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-slate-teal bg-slate-teal/10 rounded-full border border-slate-teal/20">Engineer Portal</span></div>
          <h2 className="mt-5 text-xl font-semibold text-white">Apply as an Engineer</h2>
          <p className="mt-1 text-sm text-white/30">
            Already have an account?{' '}
            <Link href="/engineer/login" className="text-slate-teal hover:text-teal-300 transition-colors">Sign in</Link>
          </p>
        </div>

        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="glass-card-light rounded-lg p-4 border border-slate-teal/10 space-y-3">
            <p className="text-xs text-slate-teal/70">Step 1: Connect Google Account *</p>
            {!googleIdToken ? (
              ...
              <p className="text-xs text-white/60">Connected as <span className="text-slate-teal">{googleEmail || formData.email}</span></p>
            )}
          </div>
          ...
          <div className="border-t border-white/6 pt-5">
            <h3 className="text-sm font-semibold text-slate-teal mb-4">Professional Information</h3>
            <div className="space-y-4">
              <div><label className={labelClass}>Professional License Number *</label><input name="professionalLicense" type="text" required value={formData.professionalLicense} onChange={handleChange} className={inputClass} placeholder="Engineering Registration Number" /></div>
              ...
              <div><label className={labelClass}>Specialization (Optional)</label><input name="specialization" type="text" value={formData.specialization} onChange={handleChange} className={inputClass} placeholder="e.g., Structural, Civil, MEP" /></div>
                ...
                {!formData.portfolio.trim() && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="glass-card-light rounded-lg p-4 border border-slate-teal/10">
                      <p className="text-xs text-slate-teal/60 mb-3">No portfolio? Upload your CV, cover letter, and work experience.</p>
                      <div className="space-y-3">
                        {[
                          ...
                          <div key={label}>
                            ...
                            <label className="flex items-center gap-2 cursor-pointer glass-input rounded-lg px-4 py-2.5 text-xs text-white/30 hover:border-slate-teal/30 transition-colors">
          ...
          <button type="submit" disabled={isLoading}
            className="w-full py-3 bg-slate-teal text-white font-semibold rounded-lg hover:bg-teal-600 transition-all duration-300 text-sm disabled:opacity-40 disabled:cursor-not-allowed">
            {isLoading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Submitting...</span> : 'Submit Application'}
          </button>
        </form>
      </motion.div>
    </div>
